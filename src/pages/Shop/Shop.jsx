...
export default function Shop() {
  ...
  useEffect(() => {
    ...
    if (productIdParam) {
      ...
      const foundProduct = INITIAL_PRODUCTS.find(
        (p) => p.id === prodId
      );
      ...
      // When going back to the catalog from a product detail,
      // restore scroll position
      if (prevProductIdParam) {
        const savedScroll = sessionStorage.getItem('shop_scroll_pos');

        if (savedScroll) {
          // Ensure the DOM has rendered the catalog list
          setTimeout(() => {
            window.scrollTo({
    ...
    // Track previous product value
    prevProductIdParamRef.current = productIdParam;
  }, [productIdParam]);

  const handleSelectProduct = (prod) => {
    // Only save scroll position if currently on catalog page
